    ...
    <?php
        ...
        for($list_day = 1; $list_day <= $days_in_month; $list_day++)
        {
            ...
            {   
                ...
                echo '<dt class="mec-calendar-day mec-has-event '.esc_attr($selected_day).'" data-mec-cell="'.esc_attr($day_id).'" data-day="'.esc_attr($list_day).'" data-month="'.date('Ym', $time).'"><a href="'.($this->display_all ? '#mec-calendar-events-sec-'.$link_date : '#').'" class="mec-has-event-a">'.MEC_kses::full(apply_filters('mec_filter_list_day_value', $list_day, $today, $this)).'</a></dt>';
                $events_str .= '<div class="mec-calendar-events-sec" id="mec-calendar-events-sec-'.esc_attr($this->id.'-'.$day_id).'" data-mec-cell="'.esc_attr($day_id).'" '.((trim($selected_day) != '' or $this->display_all) ? ' style="display: block;"' : '').'>'.MEC_kses::element($this->day_label($time));
                ...
                {  
                    ...
                    // MEC Schema
                    do_action('mec_schema', $event);
                    // if($this->main->is_multipleday_occurrence($event)) {
                        $events_str .= '<article class="'.((isset($event->data->meta['event_past']) and trim($event->data->meta['event_past'])) ? 'mec-past-event ' : '').'mec-event-article '.esc_attr($this->get_event_classes($event)).'">';
                        $events_str .= '<div class="mec-event-image">'.MEC_kses::element($event->data->thumbnails['thumblist']).'</div>';
                        ...
                        $events_str .= $after_time_filter;
                        $event_color = isset($event->data->meta['mec_color']) ? '<span class="event-color" style="background: #'.esc_attr($event->data->meta['mec_color']).'"></span>' : '';
                        $events_str .= '<h4 class="title-quaterny">'.$event->data->title;
                        if(has_filter('mec_monthly_virtual_badge')) $events_str .= apply_filters('mec_monthly_virtual_badge', '', $event->data->ID);
                        $events_str .='</h4>';
                        ...
                        if(isset($location['name'])) {
                            $events_str .= '<div class="mec-event-detail"><div class="mec-event-loc-place"><svg class="icon"><use xlink:href="#icon-marker"></use></svg>'. $location['name'].'</div></div>';
                        }
                        $events_str .= MEC_kses::element($this->booking_button($event));
                        $events_str .= MEC_kses::element($this->display_custom_data($event));
                        $events_str .= MEC_kses::element($this->display_cost($event));
                        ...
                        $events_str .= MEC_kses::full($events_filter);
                        $events_str .= '<a href="'.esc_url($this->main->get_event_date_permalink($event, $event->date['start']['date'])).'" class="mec_learn_more link link--arrow">'.esc_html__('More Info', 'modern-events-calendar-lite').'<svg class="icon"><use xlink:href="#icon-arrow"></use></svg></a>';
                        $events_str .= '</article>';
                    // }
                }
